...
const applicationRows = [
  {
    id: "260821001", type: "线上OPC", subject: "杭州线上OPC社群", organization: "杭州云栖影业有限公司",
    ...
    portfolio: "https://example.com/portfolio/hzyq", source: "线上OPC-申请加入", consent: "已同意"
  },
  {
    id: "260820018", type: "精品短剧合作", subject: "《山海有信》文旅短剧", organization: "新疆远山文旅发展有限公司",
    contact: "赵宁", phone: "186****0912", submitTime: "2026-08-20 16:18", review: "审核通过", follow: "跟进中", reviewer: "周敏",
    city: "乌鲁木齐", teamSize: "8人", model: "联合制作", scope: "文旅内容策划、景区取景协调及本地宣发资源",
    portfolio: "项目策划案、版权证明、主创团队介绍", source: "精准项目-我要合作", consent: "已同意"
  },
  {
    id: "260820011", type: "产业空间", subject: "永嘉人工智能创新应用中心", organization: "温州瓯越数字产业园",
    contact: "叶倩", phone: "159****7710", submitTime: "2026-08-20 11:06", review: "待审核", follow: "未跟进", reviewer: "-",
  ...
  },
  {
    ...
    contact: "徐老师", phone: "137****2236", submitTime: "2026-08-19 15:31", review: "审核通过", follow: "已转化", reviewer: "林楠",
    city: "杭州", teamSize: "首期60人", model: "课程共建", scope: "联合课程、真实项目进课堂、师资与算力支持",
    portfolio: "学院介绍、课程方案", source: "校企合作-我要合作", consent: "已同意"
  },
  {
    id: "260819009", type: "创作赛事", subject: "雁荡山杯文旅OPC技能大赛", organization: "青橙创作小组",
    contact: "林澈", phone: "188****4630", submitTime: "2026-08-19 10:22", review: "审核拒绝", follow: "无效", reviewer: "周敏",
    city: "南京", teamSize: "4人", model: "团队参赛", scope: "AI漫剧、文旅内容创作",
    portfolio: "作品链接无法访问", source: "创作赛事-我要参赛", consent: "已同意"
  },
  {
    id: "260818036", type: "线上OPC", subject: "成都线上OPC社群", organization: "拾光像素工作室",
    ...
    portfolio: "https://example.com/portfolio/cd", source: "线上OPC-申请创建", consent: "已同意"
  }
];
...
function statusTag(value, type) {
  const map = {
    "待审核": "warning", "审核通过": "success", "审核拒绝": "danger",
...
function applicationTabs() {
  const items = ["全部", "线上OPC", "精品短剧合作", "产业空间", "校企合作", "创作赛事"];
...
function applicationsPage() {
  ...
  const filter = [
    ...
    selectControl("申请类型", ["全部", "线上OPC", "精品短剧合作", "产业空间", "校企合作", "创作赛事"], state.applicationType, "data-application-filter"),
    inputControl("申请对象", "项目/城市/院校，模糊搜索"),
    inputControl("联系人", "请输入联系人或手机号"),
    selectControl("审核状态", ["全部", "待审核", "审核通过", "审核拒绝"], "全部"),
    selectControl("跟进状态", ["全部", "未跟进", "跟进中", "已转化", "无效"], "全部"),
  ...
  return `${applicationTabs()}
    <section class="queue-strip" aria-label="待办概览">
      <div><span>待审核</span><strong>2</strong></div><div><span>待跟进</span><strong>3</strong></div><div><span>今日新增</span><strong>4</strong></div><div><span>本月已转化</span><strong>12</strong></div>
    </section>
    ${filterPanel(filter)}
...
const docs = {
  applications: `<h2>#页面说明</h2>
    <ul><li><b>菜单路径：</b>绿台 &gt; 生态创新中心 &gt; 申请管理 &gt; 生态合作申请。</li><li><b>使用对象：</b>内容业务、生态合作运营及审核人员。</li><li>承接生态创新中心前台产生的线上OPC、精品短剧合作、产业空间、校企合作及创作赛事申请。</li></ul>
    <h3>#原型说明</h3>
    <h4>● 业务规则</h4><ul><li>用户提交成功后，绿台自动生成唯一申请ID，初始审核状态为“待审核”、跟进状态为“未跟进”。</li><li>审核状态与商务跟进状态相互独立，审核通过后仍需业务人员发起跟进。</li><li>审核操作记录审核人、审核时间、审核结论和备注，不允许直接覆盖历史记录。</li></ul>
    <h4>○ 申请类型</h4><ul><li><code>线上OPC</code>：申请加入或创建城市社群，展示团队、城市、人数、业务范围和代表作品。</li><li><code>精品短剧合作</code>：承接精准项目“我要合作”，展示项目名称、合作主体、项目介绍及合作诉求。</li><li><code>产业空间</code>：展示空间名称、城市、可承载规模和产业资源。</li><li><code>校企合作</code>：展示院校、合作方向、覆盖人数及课程/项目诉求。</li><li><code>创作赛事</code>：展示赛事名称、参赛主体、参赛作品及联系方式。</li></ul>
    <h4>○ 筛选项</h4><ul><li>申请ID为精确搜索；申请对象为模糊搜索。</li><li>申请类型、审核状态、跟进状态均为下拉单选。</li><li>提交时间支持开始日期与结束日期组合查询。</li></ul>
    <h4>○ 列表项</h4><ul><li>列表展示申请ID、申请类型、申请对象、团队/公司、联系人、联系电话、提交时间、审核状态、跟进状态和审核人。</li><li>联系电话默认脱敏，进入详情且具备权限后才展示完整号码。</li></ul>
    <h4>○ 操作项</h4><ul><li><code>详情</code>：展示本次提交的完整信息、来源入口及隐私授权状态。</li><li><code>审核</code>：仅待审核记录展示；支持审核通过或拒绝，拒绝时必填原因。</li><li><code>跟进</code>：维护跟进状态、负责人、跟进记录及下次跟进时间。</li><li><code>导出</code>：按当前筛选结果导出，需校验导出权限并记录操作日志。</li></ul>
...
function openApplicationDetail(row) {
  const body = `<div class="detail-status-line"><div><span>申请ID</span><strong>${row.id}</strong></div><div>${statusTag(row.review)} ${statusTag(row.follow)}</div></div>
    ...
    <section class="detail-section"><h3>提交与审核</h3><div class="detail-grid">
      ${displayItem("提交入口", row.source)}${displayItem("提交时间", row.submitTime)}${displayItem("隐私授权", row.consent)}${displayItem("审核人", row.reviewer)}
    </div></section>`;
  openModal("生态合作申请详情", body, `<button class="button" data-modal-cancel>关闭</button>${row.review === "待审核" ? `<button class="button primary" data-detail-review="${row.id}">审核</button>` : ""}`, "extra-wide");
...
function openReview(row) {
  ...
  openModal("审核生态合作申请", body, `<button class="button" data-modal-cancel>取消</button><button class="button primary" data-review-submit>确认审核</button>`, "wide");

...
(() => {
  ...
  const detailTables = Object.entries(schemas).map(([type, schema]) => Object.entries(schema.fields).map(([branch, fields]) => {
    const displayedFields = type === "opc" ? [{ label: "申请身份", control: "单选", rule: "必填；选项为“个人创作者”“创作团队 / 企业”，按本次提交的身份展示对应字段。" }, ...fields] : fields;
    return `<h4>${schema.label}${type === "opc" ? ` · ${branch === "personal" ? "个人创作者" : "创作团队 / 企业"}` : ""}详情</h4>${table(["官网字段（按填写顺序）", "展示规则"], displayedFields.map(item => [`${escape(item.label)}<small>${escape(item.control)}</small>`, escape(item.rule)]))}`;
  }).join("")).join("");
  window.APPLICATION_PRD_HTML = `<div class="application-prd">
    ...
    <table class="prd-changelog">
      ...
      <tbody>
        <tr><td>V1.3</td><td class="prd-date">2026-09-11</td><td>申请详情仅展示对应官网弹窗的用户填写字段，字段名称、内容及顺序与官网保持一致；OPC 保留申请身份及个人/团队条件分支。<br>移除详情中的申请 ID、申请类型、来源、提交时间及审核记录，不删除后台系统记录，不改变列表、筛选或审核功能；审核结果在列表更新。</td></tr>
    ...
    <p><b>需求：</b>按实际表单归集并展示对应字段，列表便于筛查；详情仅保留官网弹窗中的用户填写字段，不混入系统信息。删除「产业空间」Tab，不把来源社区当作用户所在地。</p>
    ...
    ${columnTables}
    <p>「申请 ID」「提交时间」「审核状态」为系统生成或运营更新的信息，继续用于列表、筛选和后台管理，不能映射成官网用户填写项。本原型使用固定演示编号、时间及审核样例；正式提交应由后台生成申请编号与提交时间，OPC 初始状态为待审核。申请类型、入口来源及审核记录仍在后台保留，不在申请详情展示，也不新增其他展示入口。</p>
    <h3>${number(4)} 操作项</h3>
    <h4>详情</h4><p>打开只读详情，仅展示该申请对应官网弹窗的用户填写字段，字段名称、选项文案和填写内容与官网一致，并按官网原有填写顺序排列，不按后台自定义分组重排。OPC 按个人/团队身份展示各自字段，精品短剧和校企合作分别使用本类表单，不混用通用「公司 / 城市 / 规模」。条件不适用字段不展示；适用但未填的字段保留并显示「未填写」，选填为空时不删除字段。关闭、遮罩或 Esc 返回原筛选结果，不改申请内容。</p>
    <p>OPC 个人/团队均在首项保留官网共用的「申请身份」。个人无「所在城市」及省市字段；团队不展示个人注册咨询、个人简介、意向团队规模等字段。个人「作品链接」「作品附件」仅在「是否有过往短剧作品」选择「有」时展示；团队作品字段始终展示。「意向工位数」仅在「是否有意向入驻 OPC 实体空间」选择「是」时展示。多选原样保留，个人与团队的枚举文案不自动合并。下表为完整字段顺序，条件项仅在满足上述条件时展示。</p>
    ${detailTables}
    <h4>OPC 审核</h4><ul><li>仅 OPC 待审核记录展示「审核」。选择审核通过 / 审核拒绝；拒绝时必须选择原因：申请资料不完整、联系方式无效、不符合当前合作范围、其他。选择其他时必须填写备注；备注最多 500 字。</li><li>「取消」、关闭、Esc 不保存。提交前校验必填项，失败时停留弹窗并显示错误；确认弹窗展示申请人与最终审核结果，二次确认后保存状态、审核人、时间、拒绝原因及备注。</li><li>演示保存仅在当前页面会话内生效，审核结果在列表更新；刷新页面恢复样例。审核完成后移除审核入口；若筛选条件已不匹配，该记录从当前结果中移除。成功提示明确标注“仅本地演示”。</li></ul>

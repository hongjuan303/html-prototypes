...
const waterfallRows = [
  ["10915", "Seedance 2.5，重新定义动态叙事", "图片", "banner-seedance-2-5.png", "不跳转", "2026-08-04 18:00", "2026-10-31 23:59", "国内"],
  ["10914", "无限画布，让创作从此没有边界", "图片", "banner-infinite-canvas.png", "https://aiqa.weiduanju.com/drama-studio", "2026-08-04 18:00", "2026-12-31 23:59", "国内"],
...
const state = { page: location.hash.slice(1) || "novels", detailTab: "roles", checked: new Set(), waterfallType: "首页轮播" };
...
function novelsPage() {
  return `${filterPanel([["内容版本","select",["国内","海外"]],["小说ID","input","请输入小说ID"],["小说名称","input","请输入小说名称"],["责编","select",["取小说管理责编枚举"]],["部门","select",["内容一部","内容二部","IP运营部"]],["状态","select",["上架","下架"]]])}
  <section class="content-card"><div class="toolbar"><button class="button primary" data-action="novel-add">${icon("plus")}新增小说</button><button class="button danger" data-action="batch-delete" ${state.checked.size ? "" : "disabled"}>${icon("trash-2")}批量删除</button><button class="button" data-action="sort">${icon("arrow-up-down")}排序</button><span class="spacer"></span><span>已选 ${state.checked.size} 项</span></div><div class="table-wrap"><table><colgroup><col style="width:42px"><col style="width:72px"><col style="width:170px"><col style="width:70px"><col style="width:76px"><col style="width:90px"><col style="width:110px"><col style="width:82px"><col style="width:72px"><col style="width:72px"><col style="width:72px"><col style="width:110px"></colgroup><thead><tr><th><input type="checkbox" data-check-all></th><th>小说ID</th><th>小说名称</th><th>封面</th><th>责编</th><th>部门</th><th>标签</th><th>试读章节</th><th>定价</th><th>内容版本</th><th>状态</th><th>操作</th></tr></thead><tbody>${novelRows.map(row => `<tr><td><input type="checkbox" data-row-check="${row[0]}" ${state.checked.has(row[0]) ? "checked" : ""}></td><td>${row[0]}</td><td class="ellipsis" title="${row[1]}">${row[1]}</td><td class="cover-cell"><img src="${asset(row[2])}" alt=""></td><td>${row[3]}</td><td>${row[4]}</td><td><span class="tag">${row[5]}</span></td><td>${row[6]}</td><td>¥${row[7]}</td><td>${versionTag(row[8])}</td><td>${switchButton(row[9], row[0], "小说")}</td><td><div class="operations"><button class="button text" data-action="novel-edit" data-id="${row[0]}">编辑</button><button class="button text danger-link" data-delete="小说" data-id="${row[0]}">删除</button></div></td></tr>`).join("")}</tbody></table></div>${pagination(125)}</section>`;
}

function scriptsPage() {
  return `${filterPanel([["内容版本","select",["国内","海外"]],["剧本ID","input","请输入剧本ID"],["剧本名称","input","请输入剧本名称"],["作者","input","请输入作者名称"],["状态","select",["上架","下架"]]])}
  <section class="content-card"><div class="toolbar"><button class="button primary" data-action="script-add">${icon("plus")}新增剧本</button><button class="button" data-action="sort">${icon("arrow-up-down")}排序</button></div><div class="table-wrap"><table><colgroup><col style="width:75px"><col style="width:170px"><col style="width:72px"><col style="width:84px"><col style="width:76px"><col style="width:110px"><col style="width:82px"><col style="width:76px"><col style="width:76px"><col style="width:72px"><col style="width:120px"></colgroup><thead><tr><th>剧本ID</th><th>剧本名称</th><th>封面</th><th>作者</th><th>男女频</th><th>标签</th><th>试读集数</th><th>定价</th><th>内容版本</th><th>状态</th><th>操作</th></tr></thead><tbody>${scriptRows.map(row => `<tr><td>${row[0]}</td><td><button class="button text title-link" data-page="scriptDetail">${row[1]}</button></td><td class="cover-cell"><img src="${asset(row[2])}" alt=""></td><td>${row[3]}</td><td>${row[4]}</td><td>${row[5].split(",").map(tag => `<span class="tag">${tag}</span>`).join("")}</td><td>${row[6]}</td><td>¥${row[7]}</td><td>${versionTag(row[8])}</td><td>${switchButton(row[9], row[0], "剧本")}</td><td><div class="operations"><button class="button text" data-action="script-edit" data-id="${row[0]}">编辑</button><button class="button text danger-link" data-delete="剧本" data-id="${row[0]}">删除</button></div></td></tr>`).join("")}</tbody></table></div>${pagination(48)}</section>`;
}
...
function scriptDetailPage() {
  ...
  return `<section class="content-card"><div class="detail-summary"><img class="detail-cover" src="${asset("work-1.jpg")}" alt=""><div class="detail-info"><h1>归来仍是掌心月</h1><div class="detail-meta"><span>${versionTag("国内")}</span><span>作者：夏月儿</span><span>女频</span><span>试读集数：3集</span><span>定价：99.00元</span></div><div class="outline"><strong>大纲</strong><br>温眠在家族品牌濒临破产时重逢旧爱陆沉。二人围绕投资协议、旧日误会与品牌发布会展开拉扯，逐步揭开当年分手真相，并在共同解决商业危机的过程中重新建立信任。</div></div><div class="detail-actions"><button class="button primary" data-action="import-script">导入剧本</button><button class="button">下载</button></div></div><div class="subtabs"><button class="subtab ${state.detailTab === "roles" ? "active" : ""}" data-detail-tab="roles">角色信息</button><button class="subtab ${state.detailTab === "episodes" ? "active" : ""}" data-detail-tab="episodes">剧集信息</button></div>${table}</section>`;
}

const docs = {
  novels: `<h2>#页面说明</h2><ul><li>本页是绿台-AI漫剧系统-AI漫剧官网配置-小说IP库配置页。</li><li>配置内容应用于官网对应的国内版或海外版小说IP库。</li></ul><h3>#原型说明</h3><p>支持从小说库选择内容，配置试读章节、定价、展示版本、排序和上下架状态。</p><h4>● 修改记录</h4><p><span class="change-date">2026-08-04</span> <span class="change-copy">新增“内容版本：国内/海外”筛选、列表字段及新增/编辑必填项；两个内容池独立排序与上下架。</span></p><h4>○ 筛选项</h4><ul><li><code>内容版本</code>：下拉单选，枚举“国内、海外”。</li><li><code>小说ID</code>：文本输入，精确搜索，仅支持正整数。</li><li><code>小说名称</code>：文本输入，模糊搜索，最多100字符。</li><li><code>责编</code>：下拉单选，枚举实时取小说管理的责编数据，不写死。</li><li><code>部门</code>：下拉单选，枚举取组织架构部门。</li><li><code>状态</code>：下拉单选，枚举“上架、下架”。</li></ul><h4>○ 列表项</h4><ul><li>小说ID、小说名称、封面、责编、部门、标签来自小说库。</li><li>试读章节：官网允许免费试读的章节数，单位“章”。</li><li>定价：小说IP展示定价，单位“元”，保留2位小数。</li><li>内容版本：决定数据展示在国内版或海外版。</li><li>状态：点击开关后弹出上架/下架二次确认。</li></ul><h4>○ 操作项</h4><ul><li>新增小说：打开居中弹窗，从小说库选择并配置必填项。</li><li>编辑：打开居中弹窗，维护试读章节、定价及内容版本。</li><li>删除/批量删除：弹出二次确认，确认后删除且不可恢复。</li><li>排序：进入当前内容版本的拖拽排序模式。</li></ul>`,
  scripts: `<h2>#页面说明</h2><ul><li>本页是绿台-AI漫剧系统-AI漫剧官网配置-剧本库配置页。</li><li>配置内容应用于官网对应的国内版或海外版剧本库。</li></ul><h3>#原型说明</h3><p>支持配置剧本基础资料、试读集数、定价、展示版本及状态。</p><h4>● 修改记录</h4><p><span class="change-date">2026-08-04</span> <span class="change-copy">新增“内容版本：国内/海外”；新增男女频、标签字段；角色信息调整至剧本详情维护。</span></p><h4>○ 筛选项</h4><ul><li><code>内容版本</code>：下拉单选，枚举“国内、海外”。</li><li><code>剧本ID</code>：文本输入，精确搜索，仅支持数字。</li><li><code>剧本名称、作者</code>：文本输入，模糊搜索，最多100字符。</li><li><code>状态</code>：下拉单选，枚举“上架、下架”。</li></ul><h4>○ 列表项</h4><ul><li>剧本ID为系统生成的纯数字唯一ID。</li><li>剧本名称以蓝字展示，点击进入剧本详情。</li><li>男女频枚举“男频、女频”；标签支持多选。</li><li>试读集数单位“集”；定价单位“元”，保留2位小数。</li><li>内容版本决定官网展示版本；状态开关需二次确认。</li></ul><h4>○ 操作项</h4><ul><li>新增/编辑剧本：打开居中弹窗，不在弹窗维护角色。</li><li>删除：弹出二次确认，确认后删除且不可恢复。</li><li>排序：进入当前内容版本的拖拽排序模式。</li></ul>`,
  ...
  waterfall: `<h2>#页面说明</h2><ul><li>本页是绿台-产品运营-内容运营-瀑布流轮播图配置页。</li><li>统一配置官网首页轮播、优质作品及素材展览。</li></ul><h3>#原型说明</h3><p>支持图片与视频素材，并按国内版、海外版分别配置。</p><h4>● 修改记录</h4><p><span class="change-date">2026-08-04</span> <span class="change-copy">无限画布 Banner 状态调整为“已上线”；新增 Seedance 2.5“即将上线”Banner；两张素材已同步至首页轮播列表。</span></p><p><span class="change-date">2026-08-04</span> <span class="change-copy">“爆款作品”统一调整为“优质作品”；新增国内/海外内容版本配置；类型新增“素材展览”。</span></p><h4>○ 筛选项</h4><ul><li><code>选择应用</code>：下拉单选，枚举“AI漫剧、其他”。</li><li><code>类型</code>：下拉单选，枚举“其他、首页轮播、优质作品、素材展览”。</li><li><code>内容版本</code>：下拉单选，枚举“国内、海外”。</li></ul><h4>○ 列表项</h4><ul><li>ID：添加素材后系统自动生成的唯一数字ID。</li><li>素材类型：图片或视频；视频展示第一帧及播放图标。</li><li>跳转方式：展示“不跳转”或网页链接。</li><li>开始时间、结束时间精确到分钟。</li><li>内容版本决定素材展示在国内版或海外版。</li></ul><h4>○ 操作项</h4><ul><li>添加/修改：打开居中弹窗。类型继承页面当前选择，不在弹窗重复填写。</li><li>选择优质作品时，额外展示“封面、热度”；热度用于官网热度值展示。</li><li>删除：弹出二次确认弹窗，确认后不可恢复。</li></ul>`
};
...
function bindInteractions() {
  ...
  document.querySelectorAll("[data-check-all]").forEach(input => input.addEventListener("change", () => { state.checked.clear(); if (input.checked) novelRows.forEach(row => state.checked.add(row[0])); render(); }));
...
function openNovelForm(edit, id) {
  const body = `${formItem("内容版本",versionSelect(edit && id === "692" ? "海外" : "国内"))}${formItem("选择小说",`<button class="upload-box" type="button" data-select-novel>${icon("book-plus")}从小说库选择</button>`)}${formItem("试读章节",`<div class="input-with-unit">${input(edit ? "8" : "","请输入正整数")}<span>章</span></div>`)}${formItem("定价",`<div class="input-with-unit">${input(edit ? "29.90" : "","保留2位小数")}<span>元</span></div>`)}`;
  openForm(edit ? "编辑小说" : "新增小说", body);
...
function openScriptForm(edit, id) {
  const body = `${formItem("内容版本",versionSelect(edit && id === "10085" ? "海外" : "国内"))}${formItem("剧本名称",input(edit ? "归来仍是掌心月" : "","最多100字符"))}${formItem("作者",input(edit ? "夏月儿" : "","请输入作者"))}${formItem("封面",`<button class="upload-box" type="button">${icon("upload")}上传封面</button>`)}${formItem("男女频",`<select class="control"><option>女频</option><option>男频</option></select>`)}${formItem("标签",`<select class="control"><option>都市、重生</option><option>玄幻、逆袭</option><option>悬疑、冒险</option></select>`)}${formItem("试读集数",`<div class="input-with-unit">${input(edit ? "3" : "","请输入正整数")}<span>集</span></div>`)}${formItem("定价",`<div class="input-with-unit">${input(edit ? "99.00" : "","保留2位小数")}<span>元</span></div>`)}`;
  openForm(edit ? "编辑剧本" : "新增剧本", body, true);
}

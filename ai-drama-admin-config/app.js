...
function contentVersionTabs() {
  return `<div class="content-version-tabs" role="tablist" aria-label="内容版本">${["国内", "海外"].map(version => `<button class="content-version-tab ${state.contentVersion === version ? "active" : ""}" type="button" role="tab" aria-selected="${state.contentVersion === version}" data-content-version="${version}">${version}</button>`).join("")}</div>`;
}
...
function waterfallPage() {
  const rows = waterfallRows.filter(row => row[7] === state.contentVersion);
  const total = state.contentVersion === "国内" ? 12 : 6;
  return `${filterPanel([["选择应用","select",["AI漫剧","其他"],"AI漫剧"],["类型","select",["其他","首页轮播","优质作品","素材展览"],state.waterfallType]])}${contentVersionTabs()}
  <section class="content-card"><div class="toolbar"><button class="button primary" data-action="banner-add">${icon("plus")}添加</button><button class="button" data-action="refresh">${icon("refresh-cw")}刷新缓存</button><button class="button" data-action="sort">${icon("arrow-up-down")}排序</button></div><div class="table-wrap"><table><colgroup><col style="width:72px"><col style="width:190px"><col style="width:86px"><col style="width:120px"><col style="width:190px"><col style="width:135px"><col style="width:135px"><col style="width:110px"></colgroup><thead><tr><th>ID</th><th>标题</th><th>素材类型</th><th>素材</th><th>跳转方式</th><th>开始时间</th><th>结束时间</th><th>操作</th></tr></thead><tbody>${rows.map(row => `<tr><td>${row[0]}</td><td class="ellipsis" title="${row[1]}">${row[1]}</td><td>${row[2]}</td><td><div class="media-thumb"><img src="${asset(row[3])}" alt="">${row[2] === "视频" ? `<span class="play">${icon("play")}</span>` : ""}</div></td><td class="ellipsis" title="${row[4]}">${row[4]}</td><td>${row[5]}</td><td>${row[6]}</td><td><div class="operations"><button class="button text" data-action="banner-edit" data-id="${row[0]}">修改</button><button class="button text danger-link" data-delete="轮播图配置" data-id="${row[0]}">删除</button></div></td></tr>`).join("")}</tbody></table></div>${pagination(total)}</section>`;
}
...
const docs = {
  ...
  scripts: `<h2>#页面说明</h2><ul><li>本页是绿台-AI漫剧系统-AI漫剧官网配置-剧本库配置页。</li><li>配置内容应用于官网对应的国内版或海外版剧本库。</li></ul><h3>#原型说明</h3><p>支持按内容版本切换剧本配置，维护基础资料、总集数、试读集数、定价及状态。</p><h4>● 修改记录</h4><p><span class="change-date">2026-08-05</span> <span class="change-copy">新增/编辑剧本弹窗新增必填字段“总集数”，单位为“集”，仅支持正整数。</span></p><p><span class="change-date">2026-08-05</span> <span class="change-copy">内容版本由筛选项、列表字段和弹窗字段调整为页面Tab；新增及上下架均作用于当前Tab对应的内容池。</span></p><p><span class="change-date">2026-08-04</span> <span class="change-copy">新增男女频、标签字段；角色信息调整至剧本详情维护。</span></p><h4>○ 内容版本Tab</h4><ul><li><code>国内</code>：展示并维护国内版官网的剧本内容，默认选中。</li><li><code>海外</code>：展示并维护海外版官网的剧本内容。</li><li>切换Tab后按对应内容池刷新列表和分页总数。</li></ul><h4>○ 筛选项</h4><ul><li><code>剧本ID</code>：文本输入，精确搜索，仅支持数字。</li><li><code>剧本名称、作者</code>：文本输入，模糊搜索，最多100字符。</li><li><code>状态</code>：下拉单选，枚举“上架、下架”。</li></ul><h4>○ 列表项</h4><ul><li>剧本ID为系统生成的纯数字唯一ID。</li><li>剧本名称以蓝字展示，点击进入剧本详情。</li><li>男女频枚举“男频、女频”；标签支持多选。</li><li>试读集数单位“集”；定价单位“元”，保留2位小数。</li><li>状态开关需二次确认。</li></ul><h4>○ 操作项</h4><ul><li>新增/编辑剧本：打开居中弹窗，内容自动归属当前版本Tab，不展示内容版本字段。</li><li>总集数：必填，正整数，单位“集”，表示当前剧本包含的完整剧集数量。</li><li>删除：弹出二次确认，确认后删除且不可恢复。</li></ul>`,
  ...
  waterfall: `<h2>#页面说明</h2><ul><li>本页是绿台-产品运营-内容运营-瀑布流轮播图配置页。</li><li>统一配置官网首页轮播、优质作品及素材展览。</li></ul><h3>#原型说明</h3><p>支持图片与视频素材，并按国内、海外两个内容池分别配置。</p><h4>● 修改记录</h4><p><span class="change-date">2026-08-07</span> <span class="change-copy">无限画布 Banner 状态更新为“正式上线”，副文案调整为“流水线画布，无限创意，自由组合，激发灵感”。</span></p><p><span class="change-date">2026-08-05</span> <span class="change-copy">删除筛选项、列表和弹窗中的“内容版本”字段；在筛选区下方新增“国内、海外”Tab，列表及新增内容自动归属当前Tab。</span></p><p><span class="change-date">2026-08-04</span> <span class="change-copy">无限画布 Banner 首次配置；新增 Seedance 2.5“即将上线”Banner；类型新增“素材展览”。</span></p><h4>○ 内容版本Tab</h4><ul><li><code>国内</code>：展示并维护国内版官网的轮播与运营素材，默认选中。</li><li><code>海外</code>：展示并维护海外版官网的轮播与运营素材。</li><li>Tab位于筛选项下方，切换后刷新对应内容池的列表和分页总数。</li></ul><h4>○ 筛选项</h4><ul><li><code>选择应用</code>：下拉单选，枚举“AI漫剧、其他”。</li><li><code>类型</code>：下拉单选，枚举“其他、首页轮播、优质作品、素材展览”。</li></ul><h4>○ 列表项</h4><ul><li>ID：添加素材后系统自动生成的唯一数字ID。</li><li>素材类型：图片或视频；视频展示第一帧及播放图标。</li><li>跳转方式：展示“不跳转”或网页链接。</li><li>开始时间、结束时间精确到分钟。</li></ul><h4>○ 操作项</h4><ul><li>添加/修改：打开居中弹窗，内容自动归属当前版本Tab。</li><li>选择优质作品时，额外展示“封面、热度”；热度用于官网热度值展示。</li><li>删除：弹出二次确认弹窗，确认后不可恢复。</li></ul>`
};
...
function openScriptForm(edit, id) {
  ...
  const body = `${formItem("剧本名称",input(edit ? (overseas ? "霓虹余烬" : "归来仍是掌心月") : "","最多100字符"))}${formItem("作者",input(edit ? (overseas ? "Maya" : "夏月儿") : "","请输入作者"))}${formItem("封面",`<button class="upload-box" type="button">${icon("upload")}上传封面</button>`)}${formItem("男女频",`<select class="control"><option>女频</option><option>男频</option></select>`)}${formItem("标签",`<select class="control"><option>${overseas ? "科幻、悬疑" : "都市、重生"}</option><option>玄幻、逆袭</option><option>悬疑、冒险</option></select>`)}${formItem("总集数",`<div class="input-with-unit"><input class="control" type="number" min="1" step="1" value="${edit ? (overseas ? "30" : "24") : ""}" placeholder="请输入正整数"><span>集</span></div>`)}${formItem("试读集数",`<div class="input-with-unit">${input(edit ? (overseas ? "5" : "3") : "","请输入正整数")}<span>集</span></div>`)}${formItem("定价",`<div class="input-with-unit">${input(edit ? (overseas ? "129.00" : "99.00") : "","保留2位小数")}<span>元</span></div>`)}`;
